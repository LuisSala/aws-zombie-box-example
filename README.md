...
* **Lab 6: Box Integration**  
    ...
    This exercise integrates Box into the application. It allows you to upload and share important documents with the other survivors.
...
In this lab, you'll integrate a Box channel with your survivor chat. Sharing important information during the apocalypse can save someone life. Survivors might need to share a picture of a new type of Zombie with you or send you a super secret government documentary with information about hidden base with medical supplies.

After completing this lab, survivors will be able to upload important files to Box and send share links with others.
...
3\. From application edit page copy:
...
4\. Go to [Account Information section in Admin console](https://app.box.com/master/settings) and Enterprise ID value.

5\. Open Lambda service in the AWS Management Console.

Click Create a Lambda function. You'll create a two Lambda functions. One for uploading a file and second for generating and posting shared link.

Skip past the blueprints page as we will not be using one. Also skip past the triggers page by selecting Next.

Give your function a name such as "[Your CloudFormation Stack name]-UploadFile". For the Nodejs version, you can keep the default Nodejs 4.3 selected.
...
13\. Now repeat the same process but instead of creating POST method create PUT. For the Lambda Function field type SharedLink.

14\. Click the Actions button on the left side of the API Gateway console and select Enable CORS.  Cllick the Enable CORS and replace existing CORS headers.

15\. Click the Actions button and select Deploy API to deploy your API. In the Deploy API window, select ZombieWorkshopStage from the dropdown and click Deploy.
...
18\. Open the **lambda/uploadFile/index.js** and **lambda/postSharedURL/index.js** file from the GitHub repo, found in the box folder.
 ...
 19\. Update all missing parameters in lambda functions under the Missing parameters information comment.
...
6\. Be sure to delete both Box Lambda functions that you created in the Box lab.
